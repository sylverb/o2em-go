...
static void do_kludges(void)
{
   switch (app_data.crc)
   {
      ...
      case 0x1C750349: /* Turtles (Europe) */
         mxsnap       = 0;
         useforen     = 1;
         break;
      case 0x06861A9C: /* Flashpoint 5 (Videopac adaptation) */
         useforen     = 1;
         setvideomode(1);
         break;
...
void write_p1(uint8_t d)
{
   ...
   {
      int i;
      int l = snapline((int)((float)master_clk/22.0+0.1), VDCwrite[0xA3], 1);
...
void ext_write(uint8_t dat, uint16_t adr)
{
   ...
   if (!(p1 & 0x08))
   {
      ...
      {
         int l = snapline((int)((float)master_clk/22.0+0.5), dat, 1);
         for (i=l; i<MAXLINES; i++)
   ...
   else if (!(p1 & 0x20)) /* Write to a Videopac+ register */
      vpp_write(dat,adr);
}
